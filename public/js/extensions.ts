...
export class StringImpl {
   ...
   * @see String.format
   */
  static print(format:string, ...params):void {
    // prepend format
    var args = [ format ];
    for (var i=0; i<params.length; ++i)
      args[i+1] = params[i];
   ...
   * @see String.sprintf
   */
  static printf(format:string, ...params):void {
    // prepend format
    var args = [ format ];
    for (var i=0; i<params.length; ++i)
      args[i+1] = params[i];
   ...
   * @returns {String} The formatted string.
   */
  static sprintf(format:string, ...params):string {
     ...
     */
    var r:RegExp = new RegExp("%(\\+)?([0 ]|'(.))?(-)?([0-9]+)?(\\.([0-9]+))?([%bcdfosxX])", "g");
   ...
   */
  static toCamelCase = function(str:string):string {
